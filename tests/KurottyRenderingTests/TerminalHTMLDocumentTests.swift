...
final class TerminalHTMLDocumentTests: XCTestCase {
    ...
    func testAWideCellIsDroppedWholeRatherThanBleedingUnderTheComposition() {
        ...
        let cells = [
            TerminalCell(character: "한", column: 1, row: 0, foreground: Fixture.white, background: Fixture.black),
        ]
    ...
    func testTheCaretSitsInsideTheCompositionRatherThanAfterIt() {
        ...
        XCTAssertEqual(TerminalHTMLDocument.cursor(frame: built).column, 2)
    }

    func testTheCaretIsTheFramesOwnColumnWhenNothingIsBeingComposed() {
        XCTAssertEqual(
            TerminalHTMLDocument.cursor(frame: frame(cells: [], cursorColumn: 7)).column,
            7
        )
    }
    ...
    func testABlockCursorFillsTheCell() {
        ...
        XCTAssertTrue(declaration.contains("width:var(\(TerminalHTMLDocument.Markup.cellWidthVariable))"), declaration)
        XCTAssertTrue(declaration.contains("height:var(\(TerminalHTMLDocument.Markup.cellHeightVariable))"), declaration)
    }
    ...
    func testABarCursorIsARuleOnTheLeadingEdge() {
        ...
        XCTAssertTrue(
            declaration.contains("width:var(\(TerminalHTMLDocument.Markup.cursorBarWidthVariable))"),
    ...
    func testAnUnderlineCursorIsARuleOnTheBottomEdge() {
        ...
        XCTAssertTrue(
            declaration.contains("height:var(\(TerminalHTMLDocument.Markup.cursorUnderlineHeightVariable))"),
            "an underline drawn a cell tall is a block, got \(declaration)"
        )
        XCTAssertTrue(
            declaration.contains(
                "calc(var(\(TerminalHTMLDocument.Markup.cellHeightVariable)) * 2 "
                    + "+ var(\(TerminalHTMLDocument.Markup.cellHeightVariable)) "
                    + "- var(\(TerminalHTMLDocument.Markup.cursorUnderlineHeightVariable)))"
            ),
    ...
    func testTheCursorIsPlacedInCellUnits() {
        ...
        XCTAssertTrue(
            declaration.contains("translate(calc(var(\(TerminalHTMLDocument.Markup.cellWidthVariable)) * 9)"),
            declaration
        )
    }
    ...
    func testAHiddenCursorIsNotDrawnAtTheTopOfTheScreen() {
        ...
        let hidden = TerminalHTMLDocument.cursor(frame: frame(cells: [], cursorRow: -1))
