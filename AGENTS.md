...
- **Title reports are off by default.** `CSI 21 t` and `CSI 20 t` answer only while `terminal.titleReportsEnabled` is on, and that key defaults to `false`. A program sets the title with OSC 0/1/2 and the report comes back on the shell's *standard input*, so the pair is a write-then-read primitive that lets screen content be replayed into the shell as typed input. Do not flip the default, do not add a second path that answers a title query, and do not gate it on anything weaker than the setting.
- **Any title that is ever reported back is control-stripped and length-capped.** Stripping removes C0 (including a bare `ESC`), DEL, and C1 — a title carrying `\n` or `\r` submits a line at the prompt, and one carrying `ESC` or `0x9b` opens an escape sequence inside Kurotty's own reply. The cap bounds a title that is entirely printable but enormous. Both live in `TerminalTitleReports.sanitized(_:)`, which the framing helper applies on the way out so no caller can frame an unsanitized title; the opt-in removes the read, never the hardening. Reporting a title from anywhere else means routing it through that same helper, not writing a second one.
- The XTWINOPS title stack (`CSI 22 ; Ps t` push, `CSI 23 ; Ps t` pop) is deliberately *not* behind that setting: it moves a title Kurotty already accepts and sends nothing back to the child. Kurotty carries one title per surface, so the icon/window selector addresses one stack, and that stack is depth-bounded because a push is child input.
...
- That rule is about **input**: keystrokes the user is typing, whose composition state lives in `setMarkedText` preedit. It does not cover **output**. macOS filesystems store filenames decomposed, so `ls`, `git log`, and every tool that prints a path emit Korean as conjoining jamo; `TerminalOutputInterpreter` composes those into precomposed syllables through `TerminalHangulComposition` before a cell is written, because three cells per syllable is the wrong glyph and the wrong column count. Screen cells, the scrollback serializer, and the search query are all in that precomposed form. Nothing Kurotty sends back to the shell — typed text, paste, path insertion — is normalized: the filesystem's bytes are authoritative there, and a normalized path names a file that does not exist.
...
- Automatic update is an existing compatibility contract, not a new optional feature. It shipped in `c5a79a9` and automatic installation was enabled in `6bb5733`; a fresh DMG install does not prove that this older behavior still works.
- Changes to Sparkle, app signing, bundle identity/versioning, appcast generation, release packaging, artifact verification, or local installation must preserve the full running-app update lifecycle: discover update, download, authenticate, quit the old process, replace the bundle, and relaunch the new version.
- Never use `codesign --deep` to create or replace a signature. It is allowed only for verification. Sign Sparkle's nested XPC services, updater app, and tools from the inside out, preserve their identifiers and entitlements, then sign the framework and host app. Every signed Sparkle component must have the same Team ID as the host app in a Developer ID release.
- Nothing except Sparkle's installer may replace `/Applications/kurotty.app` while that bundle's executable is running. Development installers must refuse before deletion or copying; they must not silently invalidate the live process's macOS code identity.
- `./scripts/check-update-contract.sh` is the fast source-level gate for these invariants. Do not weaken or bypass it to land a packaging change; update the implementation and the gate together only when the updater architecture intentionally changes.
- A release that touches the automatic-update surface requires an upgrade smoke from the immediately previous published DMG to the candidate release. Launch the previous signed app, trigger its real Sparkle update, accept it, and verify that the old process exits, the installed bundle version advances, and the candidate relaunches. Testing only a fresh install is insufficient. If this smoke cannot run, report the release as update-unverified rather than complete.
- The git tag is the single source of truth for a Kurotty release version. There is no `VERSION` file; `scripts/version.sh` resolves the version from an explicit argument, then `git describe`, so a release needs a tag and nothing else. Keeping a checked-in file in sync cost a commit and two pull requests before every release, and the tag already decided the version anyway — the workflow passes `${GITHUB_REF_NAME#v}` to the package script and the verifier compares the built `Info.plist` against that same tag.
- Do not hardcode future release numbers such as `.5` in `README.md`, scripts, workflows, tests, or app code. Tag first, then let scripts and docs derive names from the tag.
...
- The normal public release flow is:
  ...
  - create the tag from `main` as `v<next version>`;
...
- `scripts/install-app.sh` and `scripts/package-release.sh` must resolve `CFBundleShortVersionString` through `scripts/version.sh`, so an explicit argument wins and an argument-less run falls back to the tag on `HEAD`.
