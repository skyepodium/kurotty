...
- Release assets must be Universal DMGs named `kurotty-$VERSION-macos-universal.dmg`, plus `SHA256SUMS`.
...
- After release packaging changes, verify:
  ...
  - `./scripts/package-release.sh` creates `dist/kurotty-$(cat VERSION)-macos-universal.dmg`.
