...
- `MainMenu` wires app, file, split, tab, edit, workspace, search, copy-mode, quick-terminal, and preferences actions.
- `TerminalWindowController` owns the browser-like tab chrome while menu and command-palette presentation are delegated through `AppDelegate`.
...
| --- | --- | --- |
| Resize | `TerminalResizeTrace` records requested winsize and optional view/cell measurements. `TerminalResizeCycleSnapshot` compares the derived grid with PTY, screen, and renderer sizes. `TerminalResizeLedger` retains a bounded live sequence of resize snapshots. | Resize wiring must measure the AppKit viewport, derive rows and columns from cell metrics, apply PTY winsize, resize screen state, and invalidate renderer state. Ledger snapshots are diagnostics only; they do not choose the winning size or mutate terminal state. |
| Event flow | `TerminalEventLedger` groups metadata-only events under a `TerminalEventTraceID`: PTY read byte counts, parser event kinds, screen mutation counts, and render-frame metadata. `TerminalSessionRuntimeEvent` lets the PTY session publish metadata-only live boundary events without exposing raw bytes to diagnostics consumers. `TerminalTraceTimelineSummary` turns correlation reports into production-friendly stage, sequence, resize-issue, and count summaries. | The ledger may tie PTY, parser, screen, and renderer stages together for debugging. It must not store raw bytes, terminal text, pasted text, command output, or environment values. |
...
| Workspace restore | `WorkspaceSnapshot`, `WorkspaceSnapshotCoordinator`, `WorkspaceSnapshotStore`, and `WorkspaceSnapshot.restorePlan` keep layout restore separate from process restore and command replay candidates. | Layout restore may rebuild windows, tabs, splits, focus, titles, profile metadata, and cwd hints. Process restore, command replay, and agent resume remain explicit opt-in actions with inspectable risk metadata. |
