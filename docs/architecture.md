...
| Agent hook entries | `AgentStatusHookInstaller`, `AgentStatusHookCoordinator` | Writes files Kurotty does not own; per-agent consent, marker-scoped, backed up before every write. Claude Code and Codex only. |
...
| Rate-limit quota | `AgentRateLimitQuota` | Derived from transcript fields only. No API call, no network, and no credential of any kind is read. Codex only; Claude Code writes no such field. |
...
`AgentStatusHookInstaller` manages two agents and no others: `UserPromptSubmit`, `Notification`, and `Stop` in Claude Code's `~/.claude/settings.json`, and `UserPromptSubmit` and `Stop` in Codex's `~/.codex/hooks.json`. The two files share a document shape, so `AgentStatusHookTarget` carries the differences — path, event set, reported agent name, and how much of the surrounding document may be rewritten — and one set of transforms serves both. It installs no tool-use hook and ignores the agent's hook stdin payload, so no conversation content crosses the boundary; the emitted command posts a fixed body assembled from PTY environment variables and no-ops outside Kurotty. Entries are identified by a command marker so uninstall cannot remove a user's own hook, and the file is backed up before any write.

Codex has no counterpart to Claude Code's `Notification`, and its `PreToolUse` fires for every tool call whether or not approval is required, so `waitingForInput` has no faithful Codex source and is left unreported rather than approximated — the same refusal `AgentContextForecast` makes about a limit it cannot read. `blocked` has no lifecycle event in either agent and arrives only over OSC 9999.

Two rules keep the installer safe beside another writer. Codex is skipped entirely unless `~/.codex` already exists, so no configuration directory is planted for an uninstalled program. And a document Kurotty cannot fully recognize — unreadable, not JSON, not an object, or carrying a top-level key outside the `description`/`hooks` pair Codex accepts — is reported through the non-fatal diagnostic path and left byte-identical, for install and uninstall alike. `~/.codex/hooks.json` is commonly owned by third-party tooling such as `oh-my-codex`; a rewrite Kurotty guessed at could lose hooks it did not write. The permitted-key set is pinned to a verified Codex version, so a future top-level key costs status reporting until the set is updated rather than risking someone else's file.

Consent is recorded per agent, in `terminal.agentStatusHookConsent` and `terminal.agentStatusCodexHookConsent`. The prompt names the file it is about, so one answer cannot stand in for a file the user was never shown — a machine that gains Codex later gets its own question. The visible `terminal.agentStatusHooksEnabled` checkbox is only turned back off when every agent present on the machine has been refused, which is the one case where leaving it on would be a switch that does nothing.

`AgentStatusHookCoordinator` reaches the install path from two places: `applyStoredSetting` at launch, and the `AppSettingsStore` change notification for live apply. The notification path additionally requires that the app is running. That broadcast is reachable by anything in the process, and without the guard a test exercising some other observer of the same notification rewrites the developer's real `~/.claude` and blocks on a consent modal there is no session to answer.
...
`AgentRateLimitQuota` reports how much of an account's rate-limit window is spent and when it rolls over. It is bounded by what the agents write to disk, and the two agents differ absolutely: Codex records `rate_limits` beside every `token_count` event in its rollout, so a Codex quota costs no new file, credential, or connection — it is parsed from lines the scanner already reads. Claude Code records no rate-limit field anywhere under `~/.claude`; those numbers exist only behind the account's OAuth credentials, and Kurotty does not read them, so a Claude entry stays unreported and says so rather than guessing. A window is named by its own reported duration rather than classified into a fixed session/weekly pair, because Codex has already moved a plan's weekly window into the slot a positional reader would call the session one. Readings whose reset has already passed are dropped instead of displayed: the index holds every transcript on disk, so most stored readings describe a window that rolled over long ago, and a stale percentage is worse than none.
...
- live-applied: `terminal.agentSessionIndexEnabled` (default on) and `terminal.agentStatusHooksEnabled` (default on, gated by per-agent consent recorded in `terminal.agentStatusHookConsent` and `terminal.agentStatusCodexHookConsent`). Turning the index off drops every indexed record immediately; turning hooks off stops the listener and removes Kurotty's marked hook entries from every agent.
