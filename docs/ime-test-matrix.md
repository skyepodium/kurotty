...
| Focused pane input source change during composition | Focused pane has active marked `안`, Korean 2-set | Press 한/영 to switch to English, then type `dks` | The live preedit is committed to the PTY at switch time (`ptyWrite source=inputSourceChange`), the overlay clears without a frame where `안` vanishes and reorders, and IMK's late duplicate `insertText("안")` is swallowed; `keyDown` also commits if the notification arrives after the first English keystroke | Exactly `안dks`; never `dks안`, a dropped `안`, or a duplicated `안안` |
