...
final class SplitTerminalView: NSSplitView {
    ...
    var needsInitialRebalance = false
    var tmuxLayoutProportions: [CGFloat]?
    var tmuxProportionConstraints: [NSLayoutConstraint] = []
    var pendingSlotReplacementProportions: [CGFloat]?
    ...
    func activePane() -> TerminalPaneView? {
        for subview in arrangedSubviews {
            if let pane = subview as? TerminalPaneView, pane.ownsFirstResponder {
                return pane
            }
    ...
    func firstPane() -> TerminalPaneView? {
        for subview in arrangedSubviews {
            if let pane = subview as? TerminalPaneView {
                return pane
            }
